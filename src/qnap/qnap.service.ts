...
import { QNAP_ERROR_MAP } from '../constants/qnap.constant';
import {
  ApiResponse,
  DeleteOptions,
  ListOptions,
...
export class QnapService implements OnModuleDestroy {
  ...
  private async requestWithAuth(
    ...
    try {
      ...
      if (data.status === 3 && retry) {
        this.logger.warn('Session expired (Status 3). Re-authenticating...');
  ...
  private checkStatus(data: ApiResponse, context: string): void {
    ...
    if (data.status !== undefined && data.status !== 1) {
      const errorMessage =
        QNAP_ERROR_MAP[data.status] || `API returned status ${data.status}`;
  ...
  async login(): Promise<string> {
    ...
    try {
      const response = await this.http.get<string>('/cgi-bin/authLogin.cgi', {
        params: {
          user: username,
          pwd: encodedPassword,
        ...
        paramsSerializer: (
          ...
          return this.serializeParams(params as QnapRequestParams, ['pwd']);
        },
      });
      ...
      if (!sid) {
        throw new QnapApiError('login', 0, 'Login passed but no SID returned');
      }

      await this.sessionStore.setSid(sid);
      this.logger.log(`Logged in successfully. SID: ${sid.substring(0, 5)}...`);
  ...
  async copy(
    ...
    if (options.sourcePort !== undefined) base.source_port = options.sourcePort;
    if (options.destPort !== undefined) base.dest_port = options.destPort;
    if (options.waitSec !== undefined) base.wait_sec = options.waitSec;
    if (options.restoreLog !== undefined) base.restore_log = options.restoreLog;
